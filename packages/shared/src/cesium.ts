 ...
 */
export function getPositionByGeoJSON(
  ...
  try {
    // 提取 geometry 对象
    const geometry =
      (geojson as GeoJSONFeature).type === 'Feature'
...
export function getCurrentMousePosition(
  ...
  try {
    ...
    console.error('scene.pick exception when picking position')
  }
    ...
    if (pcEntity) {
      if (pcEntity.show) {
        ...
        if (cartesian) {
          ...
          console.warn('Picked the excluded noPickEntity model')
        }
      }
    } else {
