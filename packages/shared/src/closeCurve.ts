 ...
 * 闭合曲面配置选项
 */
export interface CloseCurveOptions {
  /** 平滑系数，默认 0.3 */
  smoothFactor?: number
  /** 曲线分段数，默认 100 */
  segments?: number
 ...
 * @param options 闭合曲面配置选项
 * @returns 闭合曲面的边界点
 */
export function computeCloseCurvePositions(
  positions: Cesium.Cartesian3[],
  options?: CloseCurveOptions
): Cesium.Cartesian3[] {
  ...
  for (let i = 0; i < pnts.length - 2; i++) {
    const normalPoints = getBisectorNormals(opts.smoothFactor, pnts[i], pnts[i + 1], pnts[i + 2])
