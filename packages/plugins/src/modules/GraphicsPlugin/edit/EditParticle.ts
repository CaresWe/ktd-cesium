 ...
 * 用于编辑粒子系统的位置
 */
export class EditParticle extends EditBase {
  /**
   * 外部更新位置
   */
  setPositions(position: Cesium.Cartesian3 | Cesium.Cartesian3[]): void {
    try {
      ...
      if (!position) {
        ...
        console.error('EditParticle.setPositions:', error.message)
        throw error
      }
      ...
      if (Array.isArray(position)) {
        ...
        if (position.length > 1) {
          console.warn('EditParticle.setPositions: 粒子系统编辑只支持单个位置，将使用第一个位置')
          finalPosition = position[0]
        }
      }
      ...
      if (!(finalPosition instanceof Cesium.Cartesian3)) {
        ...
        console.error('EditParticle.setPositions:', error.message, finalPosition)
        throw error
      }
  ...
  protected bindDraggers(): void {
    try {
      ...
      if (!this.entity) {
        const error = new Error('实体对象不存在')
        console.error('EditParticle.bindDraggers:', error.message)
        throw error
      }
      ...
      const dragger = draggerCtl.createDragger(this.dataSource, {
        ...
        onDrag: (_dragger: Cesium.Entity, newPosition: Cesium.Cartesian3) => {
          try {
            ...
            console.error('EditParticle.onDrag: 拖拽更新位置失败', error)
      ...
      console.error('EditParticle.bindDraggers: 绑定拖拽点失败', error)
      throw error // 向上抛出错误
    }
  }
  ...
  protected finish(): void {
    try {
      if (!this.entity) {
        console.warn('EditParticle.finish: 实体对象不存在，跳过清理')
        return
      }

      this.entity.draw_tooltip = undefined
    } catch (error) {
      console.error('EditParticle.finish: 清理失败', error)
