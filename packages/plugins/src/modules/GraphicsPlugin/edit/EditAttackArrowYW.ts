...
import { computeAttackArrowYWPositions } from '@auto-cesium/shared'
import type { ExtendedEntity } from './EditBase'
import type { AttackArrowYWEditEntity } from '../types/index'

/**
 * 攻击箭头(燕尾)编辑类
 * 用于编辑由多个点确定的攻击箭头(燕尾)
 */
export class EditAttackArrowYW extends EditPolygonEx {
  declare entity: ExtendedEntity & AttackArrowYWEditEntity
  ...
  protected override getShowPositions(
    ...
    try {
      ...
      if (positions.length < 3) {
        console.warn('EditAttackArrowYW.getShowPositions: 攻击箭头(燕尾)需要至少3个点')
        return positions
      }
      ...
      console.error('EditAttackArrowYW.getShowPositions: 获取显示位置失败', error)
      // 返回原始位置作为备选
      return positions
    }
  }
