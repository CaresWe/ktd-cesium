...
export interface ModelEntityAttr {
  ...
  pitch?: number
  roll?: number
  color?: Cesium.Color | Cesium.Property
  silhouetteColor?: Cesium.Color | Cesium.Property
  silhouetteSize?: number
...
export function style2Entity(style?: ModelStyleConfig, entityattr?: ModelEntityAttr): ModelEntityAttr {
  ...
  // 透明度、颜色
  const opacity = finalStyle.opacity !== undefined ? finalStyle.opacity : 1
  if (finalStyle.fill) {
    entityattr.color = Cesium.Color.fromCssColorString(finalStyle.color || '#FFFFFF').withAlpha(opacity)
  } else {
    entityattr.color = Cesium.Color.fromCssColorString('#FFFFFF').withAlpha(opacity)
  }
