...
function ensureMaterialProperty(material: unknown): Cesium.MaterialProperty | undefined {
  ...
  console.error(
    ...
    material && typeof material === "object" && material.constructor
...
function setFillMaterial(entityattr: EntityAttr, style: StyleConfig): EntityAttr {
  ...
  if (style.color || style.fillType) {
    ...
    const color = Cesium.Color.fromCssColorString(colorString).withAlpha(
    ...
    switch (style.fillType) {
      ...
      case "checkerboard": // Checkerboard
        {
          ...
          const oddColorString = typeof style.checkerboard_oddcolor === 'string'
            ? style.checkerboard_oddcolor
            : "#ffffff";
          entityattr.material = new Cesium.CheckerboardMaterialProperty({
            evenColor: color,
            oddColor: Cesium.Color.fromCssColorString(oddColorString).withAlpha(
              Number(style.opacity ?? 1.0)
            ),
      ...
      case "stripe": // Stripe
        {
          const stripeOddColorString = typeof style.stripe_oddcolor === 'string'
            ? style.stripe_oddcolor
            : "#ffffff";
          entityattr.material = new Cesium.StripeMaterialProperty({
            evenColor: color,
            oddColor: Cesium.Color.fromCssColorString(stripeOddColorString).withAlpha(
              Number(style.opacity ?? 1.0)
            ),
      ...
      case "animationLine": // Flow line
        if (style.animationImage && typeof style.animationImage === 'string') {
          const bgColorValue = style.bgColor && typeof style.bgColor === 'string'
      ...
      case "animationCircle": // Dynamic circle
        {
          ...
          entityattr.material = new CircleWaveMaterial({
            duration: style.animationDuration ?? 2000,
            color: Cesium.Color.fromCssColorString(circleColorString).withAlpha(
              Number(style.opacity ?? 1.0)
            ),
  ...
  for (const key in style) {
    ...
    switch (key) {
      ...
      case "outlineColor": // Outline color
        {
          const outlineColorValue = typeof value === 'string'
            ? value
            : (typeof style.color === 'string' ? style.color : "#FFFF00");
          entityattr.outlineColor = new Cesium.ConstantProperty(
            Cesium.Color.fromCssColorString(outlineColorValue).withAlpha(
