  ...
  if (material && typeof material === 'object') {
    console.error(
      ...
      (material as { constructor?: { name: string } }).constructor ?
        (material as { constructor: { name: string } }).constructor.name : "no constructor"
    );
...
export function style2Entity(style?: PolylineStyleConfig, entityattr?: PolylineEntityAttr): PolylineEntityAttr {
  ...
  if (finalStyle.color || finalStyle.lineType) {
    const color = Cesium.Color.fromCssColorString(
      finalStyle.color || "#FFFF00"
    ).withAlpha(Number(finalStyle.opacity ?? 1.0));
        ...
        if (finalStyle.outline) {
          ...
          entityattr.material = new Cesium.PolylineOutlineMaterialProperty({
            ...
            outlineColor: Cesium.Color.fromCssColorString(
              finalStyle.outlineColor || "#FFFF00"
            ).withAlpha(Number(finalStyle.outlineOpacity || finalStyle.opacity || 1.0)),
          });
        } else {
...
export function getPositions(entity: PolylineEntity, isShowPositions?: boolean): Cesium.Cartesian3[] {
  if (!isShowPositions && entity._positions_draw && entity._positions_draw.length > 0)
    return entity._positions_draw; // For curved lines, get bound data
 ...
 */
export function toGeoJSON(entity: PolylineEntity, _coordinates?: number[][]): GeoJSONFeature {
